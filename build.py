...
for f in concept_dir.glob("*.yml"):
    ...
    cid = c["id"]
    if cid in ids:
        print("Duplicate id:", cid); sys.exit(1)
    ids.add(cid)
    uri = base_uri + cid
    ...
    if defs: obj["skos:definition"] = defs
...
(public_dir / "sanasto.jsonld").write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding="utf-8")
print("OK built", len(concepts))
